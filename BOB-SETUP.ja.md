...
> Bob の実設定ファイルと突き合わせて確認済みです。ポリシーの適用については、
> **IBM Bob IDE で動作を確認しました**（macOS / arm64 / Bob 2.0.2）。
> **Bob Shell（headless の `bob run`）については未検証**です。
> ポリシーの検証は IDE で行ってください。
>
> 確認できた内容:
>
> - `sudo profiles install` した `.mobileconfig` の `EnforcedHooks` が IDE セッションで実行された
> - ポリシー側の hook とユーザー設定側の hook が**両方走り、同一セッションが二重に記録された**
>   （[6-5](#6-5-展開前チェックリスト) の二重計上の注意は実測済みです）
...
> **ポリシーの検証は IBM Bob IDE で行ってください。**
> `EnforcedHooks` が実際に適用されることは IDE で確認しました
> （macOS / arm64 / Bob 2.0.2）。headless の `bob run`（Bob Shell）では
> 発火を確認できていません（未検証）。手順を検証するときは IDE を使ってください。
>
...
- [ ] 利用者側に既存の登録が**無い**（あると二重計上。[6-3](#6-3-ポリシーに設定する)）
      — **実機で確認済み**：IDE の1セッションが、ポリシー側とユーザー設定側の
      両方に 17 span ずつ、計 34 span 記録されました
...
1. **1台で検証** — ポリシーを自分の端末だけに適用し、**IBM Bob IDE で**1ターン動かして
   [5章](#5-動作確認)の確認をすべて通す。特に
   「プロンプトに JSON が混入していないこと」を目視する。
   ポリシーの発火確認は IDE で行ってください（headless の `bob run` は未検証です）
